...
class FirestoreChatRepository implements ChatRepository {
  ...
  Future<void> sendMessage(ChatMessage message) async {
    ...
    // Ensure room document exists with participants field.
    // SetOptions(merge: true) means if the room already exists, we only
    // add/update the participants field without overwriting anything.
    // For new rooms it creates the document immediately when the first
    // message is sent, making it queryable by the inbox screen.
    await _firestore
        .collection('chat_rooms')
        .doc(roomId)
        .set({
